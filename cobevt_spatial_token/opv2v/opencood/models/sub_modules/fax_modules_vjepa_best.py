"""
fax_modules_vjepa_best.py

GroupNorm-friendly FAX module for V-JEPA spatial features.

Place at:
    opencood/models/sub_modules/fax_modules_vjepa_best.py

This is adapted from your single-agent/FAX implementation. Main difference:
BatchNorm in the FAX projection/BEV residual blocks is replaced by GroupNorm
by default, which is typically more stable for frozen/LoRA foundation features.
"""
...
def Norm2d(channels: int, norm_type: str = "gn"):
...
class BEVResidualBlock(nn.Module):
    def __init__(self, channels: int, norm_type: str = "gn"):
        super().__init__()
        mid = max(channels // 4, 16)
        self.net = nn.Sequential(
            nn.Conv2d(channels, mid, 1, bias=False),
            Norm2d(mid, norm_type),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid, mid, 3, padding=1, bias=False),
            Norm2d(mid, norm_type),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid, channels, 1, bias=False),
            Norm2d(channels, norm_type),
        )
...
class FAXModule(nn.Module):
    def __init__(
        ...
        cross_view_swap = dict(config['cross_view_swap'])
        norm2d_type = config.get('norm2d', 'gn')
        cross_view_swap.setdefault('norm2d_type', norm2d_type)
        ...
        for i, (feat_shape, num_layers) in enumerate(zip(self.backbone_output_shape, middle)):
            ...
            layer = nn.Sequential(*[BEVResidualBlock(dim[i], norm2d_type) for _ in range(num_layers)])
            layers.append(layer)
