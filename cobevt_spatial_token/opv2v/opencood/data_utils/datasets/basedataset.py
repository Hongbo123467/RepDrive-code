...
class BaseDataset(Dataset):
    ...
    def __init__(self, params, visualize, train=True, validate=False):
        ...
        if isinstance(fusion_args, list):
            fusion_args = {}   # 兼容旧版 args: [] 写法

        self.queue_length          = fusion_args.get('queue_length', 1)
        self.timestamp_offset      = fusion_args.get('timestamp_offset', 0)
        self.timestamp_offset_mean = fusion_args.get('timestamp_offset_mean', 0)
        self.timestamp_offset_std  = fusion_args.get('timestamp_offset_std', 0)
    ...
    def retrieve_base_data(self, idx, cur_ego_pose_flag=True):
        ...
        if isinstance(idx, int):
            scenario_database, timestamp_indices = self.retrieve_by_idx(idx)
        elif isinstance(idx, tuple):
            scenario_database = self.scenario_database[idx[0]]
            timestamp_indices = np.array([idx[1]])  # 单帧兼容
        else:
            import sys
            sys.exit('Index has to be a int or tuple')

        data_queue = []   # 时间序列，索引 0=当前帧，后续为历史帧

        from opencood.utils.transformation_utils import calculate_prev_pose_offset

        for q_i, timestamp_index in enumerate(timestamp_indices):
            ...
            for cav_id, cav_content in scenario_database.items():
                ...
                if q_i == 0:
                    ...
                    prev_data = data_queue[q_i - 1]
                    if cav_id in prev_data:
                        data[cav_id]['prev_bev_exists'] = True
                        data[cav_id]['prev_pose_offset'] = \
                            calculate_prev_pose_offset(data[cav_id], prev_data[cav_id])
                    else:
                        data[cav_id]['prev_bev_exists'] = False
                        data[cav_id]['prev_pose_offset'] = np.eye(4)
                
                # 仅对序列第一帧（当前帧）加载额外真值信息
                if q_i == 0:
                    for file_extension in self.add_data_extension:
        ...
        # 单帧模式（queue_length=1）返回原始格式，兼容旧逻辑
    ...
    def retrieve_by_idx(self, idx):
        ...
        Returns
        -------
        scenario database and timestamp indices.
        """
        # we loop the accumulated length list to see get the scenario index
        scenario_index = 0
        ...
        # ── 历史帧：生成时间序列索引 ──────────────────────────
        # 确定实际时间偏移（支持训练时随机增强）
        if self.train and self.timestamp_offset_mean > 0 and self.timestamp_offset_std > 0:
            timestamp_offset = max(0, int(
                np.random.normal(self.timestamp_offset_mean, self.timestamp_offset_std)
            ))
        else:
            timestamp_offset = self.timestamp_offset

        # 时间序列跨度 = (T-1)*(offset+1) + 1
        # 当前帧在前，历史帧在后
        # indices = [t, t - (offset+1), ..., t - (T-1)*(offset+1)]
        step = timestamp_offset + 1
        timestamp_indices = np.array([
            max(0, timestamp_index - i * step)
            for i in range(self.queue_length)
        ])

        return scenario_database, timestamp_indices
