...
def main():
    ...
    rank, world_size = multi_gpu_utils.get_dist_info()
    ...
    # record training
    writer = SummaryWriter(saved_path) if is_main_process else _NoopWriter()
    ...
    for epoch in range(init_epoch, max(epoches, init_epoch)):
        ...
        if epoch % hypes['train_params']['eval_freq'] == 0:
            ...
            with torch.no_grad():
                for i, batch_data in enumerate(val_loader):
                    ...
                    iou_dynamic, iou_static = cal_iou_training(batch_data,
                                                               output_dict)
                    metric_sums += torch.tensor([final_loss.item(),
                                                 iou_dynamic[1],
                                                 iou_static[1],
                                                 iou_static[2],
                                                 1.0],
            ...
            metric_count = max(metric_sums[4].item(), 1.0)
            valid_ave_loss = (metric_sums[0] / metric_count).item()
            dynamic_ave_iou = (metric_sums[1] / metric_count).item()
            static_ave_iou = (metric_sums[2] / metric_count).item()
            lane_ave_iou = (metric_sums[3] / metric_count).item()
        ...
        if is_main_process and epoch % hypes['train_params']['save_freq'] == 0:
