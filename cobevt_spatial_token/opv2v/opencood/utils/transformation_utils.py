...
def calculate_prev_pose_offset(current_frame_data, prev_frame_data):
    """
    计算前一帧到当前帧的 ego 位姿变换矩阵（4x4）。

    Parameters
    ----------
    current_frame_data: dict
        当前帧单 CAV 数据字典
    prev_frame_data: dict
        前一帧单 CAV 数据字典

    Returns
    -------
    T_prev_to_cur: np.ndarray
        位姿变换矩阵
    """
    cur_pose = current_frame_data['params']['lidar_pose']
    prev_pose = prev_frame_data['params']['lidar_pose']
    # 从 prev_pose 变换到 cur_pose
    return x1_to_x2(prev_pose, cur_pose)
