...
class DrivoRTargetBuilder(AbstractTargetBuilder):
    ...
    def compute_targets(self, scene: Scene) -> Dict[str, torch.Tensor]:
        ...
        extra_targets = {}
        if hasattr(self._config, "num_bounding_boxes"):
            agent_states, agent_labels = self._compute_agent_targets(annotations)
            extra_targets["agent_states"] = agent_states
            extra_targets["agent_labels"] = agent_labels
        if hasattr(self._config, "bev_semantic_classes"):
            extra_targets["bev_semantic_map"] = self._compute_bev_semantic_map(
    ...
    @staticmethod
    def _semantic_items(config):
        classes = config.bev_semantic_classes
        return classes.items() if hasattr(classes, "items") else enumerate(classes)

    @staticmethod
    def _semantic_entry(entry):
        if hasattr(entry, "get"):
            return entry.get("entity_type"), entry.get("layers")
        return entry[0], entry[1]

    @staticmethod
    def _convert_semantic_layers(entity_type: str, layers):
        converted = []
        for layer in layers:
            if not isinstance(layer, str):
                converted.append(layer)
            elif entity_type in ("polygon", "linestring"):
                converted.append(getattr(SemanticMapLayer, layer))
            else:
                converted.append(getattr(TrackedObjectType, layer))
        return converted
    ...
    def _compute_bev_semantic_map(
        ...
        for label, entry in self._semantic_items(self._config):
            entity_type, layers = self._semantic_entry(entry)
            layers = self._convert_semantic_layers(entity_type, layers)
            label = int(label)
