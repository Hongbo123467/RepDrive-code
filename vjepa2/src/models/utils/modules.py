...
class RoPEAttention(nn.Module):
    ...
    def forward(self, x, mask=None, attn_mask=None, T=None, H_patches=None, W_patches=None, action_tokens=0, action_rope=False):
        B, N, C = x.size()
        ...
        if action_tokens > 0:
            scene_q = q[:, :, :action_tokens, :]
            scene_k = k[:, :, :action_tokens, :]
            scene_v = v[:, :, :action_tokens, :]
            
            if action_rope:
                scene_pos = torch.arange(action_tokens, device=x.device)
                scene_qd = rotate_queries_or_keys(scene_q[..., :self.d_dim], pos=scene_pos)
                scene_kd = rotate_queries_or_keys(scene_k[..., :self.d_dim], pos=scene_pos)
                scene_qr = scene_q[..., self.d_dim:]
                scene_kr = scene_k[..., self.d_dim:]
                scene_q = torch.cat([scene_qd, scene_qr], dim=-1)
                scene_k = torch.cat([scene_kd, scene_kr], dim=-1)
        ...
        spatial_N = q.size(2)
        grid_depth = int(spatial_N // (self.grid_size * self.grid_size))
        ...
        if action_tokens > 0:
            q = torch.cat([scene_q, q], dim=2)
            k = torch.cat([scene_k, k], dim=2)
            v = torch.cat([scene_v, v], dim=2)
...
class Block(nn.Module):
    ...
    def forward(self, x, mask=None, attn_mask=None, T=None, H_patches=None, W_patches=None, action_tokens=0, action_rope=False):
        if isinstance(self.attn, RoPEAttention):
            y = self.attn(self.norm1(x), mask=mask, attn_mask=attn_mask, T=T, H_patches=H_patches, W_patches=W_patches, action_tokens=action_tokens, action_rope=action_rope)
        else:
