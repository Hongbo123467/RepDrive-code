...
# ---------------------------------------------------------------------------
# V-JEPA2 VisionTransformer + Scene Token 注入
# 与 DINOv2 的 timm_ViT._pos_embed 策略完全对称：
#   在 patch_embed + pos_embed 之后、过 attention blocks 之前，
#   将可学习的 scene_tokens 拼接到序列头部。
# ---------------------------------------------------------------------------

class VisionTransformerWithSceneTokens(VisionTransformer):
    """
    在标准 V-JEPA2 VisionTransformer 基础上，支持注入外部 scene_tokens。

    scene_tokens 是在 patch 序列之前拼接的可学习查询向量（由外部传入），
    它们会参与所有 Transformer blocks 的自注意力计算，从而与图像 patch tokens
    发生充分的跨帧 / 跨视角交互。

    这与 DINOv2 (timm_ViT._pos_embed) 的实现方式完全一致：
        x = cat([scene_tokens, patch_tokens], dim=1)
        x → blocks → norm → 返回完整序列

    调用侧在 forward 结果中取前 num_scene_tokens 个 token 作为场景特征。
    """
    ...
    def forward_features(
        ...
        """
        带 scene_token 注入的前向特征提取。

        @param {Tensor} x - 输入视频/图像，形状 [B, C, T, H, W] 或 [B, C, H, W]
        @param {Tensor} scene_tokens - 可学习查询向量，形状 [B, num_scene_tokens, embed_dim]
                                       若为 None，退化为标准 VisionTransformer
        @param masks - 可选掩码（保留接口兼容性）
        @returns {Tensor} - token 序列，形状 [B, num_scene_tokens + num_patches, embed_dim]
        """
        if masks is not None and not isinstance(masks, list):
            masks = [masks]

        # ---------- 与原始 forward 完全对应的 patch embed + pos embed ----------
        if x.ndim == 4:
            _, _, H, W = x.shape
            T = 1
        elif x.ndim == 5:
            ...
            raise ValueError(f"不支持的输入维度：{x.ndim}D")
        ...
        if not self.use_rope:
            ...
            x = self.patch_embed(x)       # RoPE 模式下位置编码在 Block 内完成

        # --------- [核心] 注入 scene_tokens，与 DINOv2 timm_ViT 对称 ---------
        # 记录头部拼接的全局 scene_tokens 个数
        S = scene_tokens.size(1) if scene_tokens is not None else 0
        if S > 0:
            x = torch.cat([scene_tokens, x], dim=1)   # [B, S+N, D]

        # 可选掩码（掩码索引需相对 patch tokens，忽略 scene_tokens 部分）
        if masks is not None:
            x = apply_masks(x, masks)
            masks = torch.cat(masks, dim=0)

        # 传入 action_tokens 数量，并设置 action_rope 标志位
        for i, blk in enumerate(self.blocks):
            if self.use_activation_checkpointing:
                x = torch.utils.checkpoint.checkpoint(
                    blk, x, masks, None,
                    T=T, H_patches=H_patches, W_patches=W_patches,
                    action_tokens=S, action_rope=False,
                    use_reentrant=False
                )
            else:
    ...
    def forward(self, x: torch.Tensor, scene_tokens: torch.Tensor = None, masks=None) -> torch.Tensor:
        """
        @param {Tensor} x - 输入，形状 [B, C, T, H, W]
        @param {Tensor} scene_tokens - 场景查询向量，形状 [B, num_scene_tokens, embed_dim]
        @param masks - 可选掩码
        @returns {Tensor} - token 序列 [B, S+N, D]（前 S 个为 scene_tokens 的输出）
        """
        return self.forward_features(x, scene_tokens=scene_tokens, masks=masks)
